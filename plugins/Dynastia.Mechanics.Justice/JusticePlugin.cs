...
public sealed class JusticePlugin :
    IGamePlugin
{
    private const string CrimesPath =
        "Common/crimes.json";
        ...
        IGamePluginContext context)
    {
        ...
        var guards =
            context.GetService<IActionGuardRegistry>()
            ?? throw new InvalidOperationException(
                "Action guard registry is unavailable.");

        var crimes =
            JsonSerializer.Deserialize<
                List<CrimeDefinition>>(
                    data.ReadText(
                        CrimesPath),
            ...
            ?? throw new InvalidDataException(
                $"Could not read {CrimesPath}.");

        ValidateCrimes(
            crimes);

        var justice =
            new StandardJusticeService();

        context.AddService<IJusticeService>(
            justice);

        guards.Register(
            new PrisonActionGuard(
                justice));

        systems.Register(
            new PrisonStatusYearSystem(
                justice,
                family,
                events));
        ...
        IReadOnlyList<CrimeDefinition> crimes)
    {
            ...
            crimes)
        {
            ...
            {
                throw new InvalidDataException(
                    "Every crime needs id, name and description.");
            }
