...
public sealed class CareerViewModel
{
        ...
        bool isAlive)
    {
        ...
        JobSatisfaction = snapshot.JobSatisfaction;
        AnnualIncome = snapshot.AnnualIncome;
        IsRetired = snapshot.IsRetired;

        OccupationText =
            snapshot.JobLevel > 0
                ? $"Occupation: {snapshot.JobTitle} (Level {snapshot.JobLevel})"
        ...
        SatisfactionText =
            ...
            && !snapshot.IsRetired
            && snapshot.JobLevel > 0
                ? $"Job satisfaction: {snapshot.JobSatisfactionText}"
                : string.Empty;
        ...
        IncomeText =
            !isAlive
                ? string.Empty
                : snapshot.IsRetired
                    ? $"Pension: ${snapshot.AnnualIncome:N0}/year"
                    : snapshot.JobLevel > 0
                        ? $"Income: ${snapshot.AnnualIncome:N0}/year"
                        : string.Empty;
    }
